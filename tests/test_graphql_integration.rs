...
async fn test_graphql_simple_query() {
    ...
    assert!(
        output.contains("users") || output.contains("Alice") || output.contains("data"),
        "Should return GraphQL data. output: {}",
        output
    );
}
...
async fn test_graphql_error_response() {
    ...
    assert!(
        ...
        "Should show GraphQL errors. output: {}",
        output
    );
}
...
async fn test_graphql_partial_data_with_errors() {
    ...
    assert!(
        output.contains("users")
            ...
            || output.contains("error")
            || output.contains("data"),
        "Should handle partial data. output: {}",
        output
    );
}
...
fn test_graphql_invalid_query_syntax() {
    let response = http_error(&[
        "--graphql",
        "--graphql-query",
        "{ users { id name }", // Missing closing brace
        "http://localhost:9999/graphql",
    ]);
...
async fn test_graphql_nested_query() {
    ...
    assert!(
        output.contains("posts")
            ...
            || output.contains("data"),
        "Should return nested data. output: {}",
        output
    );
}
...
async fn test_graphql_variable_types() {
    ...
    Mock::given(method("POST"))
        .and(path("/graphql"))
        .respond_with(
            ResponseTemplate::new(200).set_body_json(serde_json::json!({"data": {"result": true}})),
        )
        .mount(&mock_server)
        .await;
...
async fn test_graphql_verbose_mode() {
    ...
    Mock::given(method("POST"))
        .and(path("/graphql"))
        .respond_with(
            ResponseTemplate::new(200).set_body_json(serde_json::json!({"data": {"test": true}})),
        )
        .mount(&mock_server)
        .await;
    ...
    assert!(
        output.len() > 50,
        "Verbose should produce more output. output: {}",
        output
    );
}
