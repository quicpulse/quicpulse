...
fn test_http_file_load_fixture() {
    ...
    assert!(
        output.contains("GET") || output.contains("POST") || output.contains("users"),
        "Should list requests. output: {}",
        output
    );
}
...
fn test_http_file_list_shows_names() {
    ...
    let output = format!("{}{}", response.stdout, response.stderr);
    assert!(
        output.contains("Get") || output.contains("users") || output.contains("1."),
        "Should show request names/indices. output: {}",
        output
    );
}
...
fn test_http_file_nonexistent() {
    let response = http_error(&[
        "--http-file",
        "/nonexistent/path/requests.http",
        "--http-list",
    ]);
...
"#,
    ...
    assert!(
        output.contains("First") || output.contains("Second") || output.contains("2"),
        "Should list multiple requests. output: {}",
        output
    );
}
...
async fn test_http_file_with_comments() {
    ...
    let http_file = temp_dir.path().join("test.http");
    let content = format!(
        r#"# This is a comment at the top
### Get test endpoint
# Another comment
GET {}/test
...
async fn test_http_file_all_methods() {
    ...
    for http_method in &["GET", "POST", "PUT", "DELETE", "PATCH"] {
        let http_file = temp_dir
            .path()
            .join(format!("test_{}.http", http_method.to_lowercase()));
...
"#;
    ...
    assert!(
        output.contains("users") || output.contains("Get") || output.contains("3"),
        "Should list named requests. output: {}",
        output
    );
}
...
fn test_http_file_empty_file() {
    ...
    assert!(
        ...
        "Should handle empty file. stderr: {}",
        response.stderr
    );
}
...
fn test_http_file_invalid_request_line() {
    ...
    assert!(
        ...
        "Should reject invalid method. stderr: {}",
        response.stderr
    );
}
...
fn test_http_file_fixture_has_correct_count() {
    ...
    assert!(
        ...
        "Should list multiple requests from fixture. output: {}",
        output
    );
}
