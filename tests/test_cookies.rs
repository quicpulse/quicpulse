...
async fn test_cookies_stored_in_session() {
    ...
    Mock::given(method("GET"))
        .and(path("/set"))
        .respond_with(
            ResponseTemplate::new(200).insert_header("Set-Cookie", "session_id=12345; Path=/"),
        )
        .mount(&server)
        .await;
...
async fn test_cookie_on_redirect() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string("Cookie from redirect received"))
        .mount(&server)
        .await;
...
async fn test_cookie_accumulated_on_multiple_redirects() {
    ...
    Mock::given(method("GET"))
        .and(path("/final"))
        .respond_with(ResponseTemplate::new(200).set_body_string("All cookies"))
        .mount(&server)
        .await;
...
async fn test_cookie_with_path() {
    ...
    Mock::given(method("GET"))
        .and(path("/api/set"))
        .respond_with(
            ResponseTemplate::new(200).insert_header("Set-Cookie", "api_cookie=value; Path=/api/"),
        )
        .mount(&server)
        .await;
    ...
    Mock::given(method("GET"))
        .and(path("/other"))
        .respond_with(ResponseTemplate::new(200).set_body_string("No cookie"))
        .mount(&server)
        .await;
