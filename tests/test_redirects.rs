...
async fn test_follow_all_redirects_shown() {
    ...
    Mock::given(method("GET"))
        .and(path("/redirect/2"))
        .respond_with(ResponseTemplate::new(302).insert_header("Location", "/redirect/1"))
        .mount(&server)
        .await;
...
async fn test_max_redirects() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(302).insert_header("Location", "/redirect/infinite"))
        .mount(&server)
        .await;
...
async fn test_307_preserves_method_and_body() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(ResponseTemplate::new(307).insert_header("Location", "/post"))
        .mount(&server)
        .await;
...
async fn test_308_preserves_method_and_body() {
    ...
    Mock::given(method("PUT"))
        .and(path("/redirect-to"))
        .respond_with(ResponseTemplate::new(308).insert_header("Location", "/put"))
        .mount(&server)
        .await;
...
async fn test_301_redirect() {
    ...
    Mock::given(method("GET"))
        .and(path("/new"))
        .respond_with(ResponseTemplate::new(200).set_body_string("New location"))
        .mount(&server)
        .await;
