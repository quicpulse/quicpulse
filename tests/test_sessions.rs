...
async fn test_session_reused() {
    ...
    Mock::given(method("GET"))
        .and(path("/get"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"headers": {}})))
        .mount(&server)
        .await;
    ...
    let r1 = http_with_env(
        &["--print=hb", "--session=test", &url, "X-Custom:value1"],
        &env,
    );
...
async fn test_session_with_auth() {
    ...
    let r1 = http_with_env(
        &["--print=hb", "--session=test", "--auth=user:password", &url],
        &env,
    );
    assert!(r1.contains(HTTP_OK));
...
async fn test_session_cookie_update() {
    ...
    Mock::given(method("GET"))
        .and(path("/set"))
        .respond_with(ResponseTemplate::new(200).insert_header("Set-Cookie", "test=value1; Path=/"))
        .mount(&server)
        .await;
    ...
    Mock::given(method("GET"))
        .and(path("/update"))
        .respond_with(ResponseTemplate::new(200).insert_header("Set-Cookie", "test=value2; Path=/"))
        .mount(&server)
        .await;
...
async fn test_session_headers() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string("Headers present"))
        .mount(&server)
        .await;
...
async fn test_session_header_overwrite() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string("Header updated"))
        .mount(&server)
        .await;
...
async fn test_anonymous_session() {
    ...
    Mock::given(method("GET"))
        .and(path("/get"))
        .respond_with(ResponseTemplate::new(200).insert_header("Set-Cookie", "anon=yes"))
        .mount(&server)
        .await;
