...
async fn test_digest_auth_type() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(401).insert_header(
            "WWW-Authenticate",
            r#"Digest realm="test@example.org", nonce="abc123""#,
        ))
        .mount(&server)
        .await;
...
fn test_missing_auth_with_auth_type() {
    let r = http(&["--offline", "--print=H", "--auth-type=basic", "example.org"]);
...
fn test_username_only_in_url() {
    ...
    let r = http(&["--offline", "--print=H", "http://username@example.org"]);
