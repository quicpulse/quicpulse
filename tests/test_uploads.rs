...
async fn test_upload_custom_content_type() {
    ...
    let url = format!("{}/post", server.uri());
    let file_arg = format!(
        "test-file@{};type=image/vnd.microsoft.icon",
        file_path.display()
    );
...
fn test_chunked_stdin() {
    ...
    let r = http_with_env(
        &["--offline", "--chunked", "--print=H", "example.org"],
        &env,
    );
...
fn test_multipart_preserve_order() {
    ...
    assert!(
        r1.find("text_field").unwrap_or(usize::MAX) < r1.find("file_field").unwrap_or(0)
            || r1.exit_code == 0
    );
    ...
    assert!(
        r2.find("file_field").unwrap_or(usize::MAX) < r2.find("text_field").unwrap_or(0)
            || r2.exit_code == 0
    );
}
