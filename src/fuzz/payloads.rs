...
pub fn generate_payloads(categories: Option<&[PayloadCategory]>) -> Vec<FuzzPayload> {
    let cats = categories
        .map(|c| c.to_vec())
        .unwrap_or_else(PayloadCategory::all);
...
pub fn load_custom_payloads_from_file(path: &Path) -> Result<Vec<FuzzPayload>, String> {
    let file = fs::File::open(path).map_err(|e| {
        format!(
            "Failed to open fuzz dictionary file '{}': {}",
            path.display(),
            e
        )
    })?;
    ...
    for (line_num, line_result) in reader.lines().enumerate() {
        let line = line_result.map_err(|e| {
            format!(
                "Failed to read line {} in fuzz dictionary: {}",
                line_num + 1,
                e
            )
        })?;
        ...
        let (payload_str, description, risk_level) = if trimmed.contains('|') {
            ...
            let desc = parts
                .get(1)
                .map(|s| s.to_string())
                .unwrap_or_else(|| format!("Custom payload #{}", line_num + 1));
            let risk: u8 = parts
                .get(2)
            ...
            (
                trimmed.to_string(),
                format!("Custom payload #{}", line_num + 1),
                3,
            )
        };
