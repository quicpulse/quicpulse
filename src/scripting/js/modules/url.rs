...
pub fn register(ctx: &Ctx<'_>) -> Result<(), QuicpulseError> {
    ...
    url_obj
        .set("parse", Function::new(ctx.clone(), url_parse)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    url_obj
        .set("host", Function::new(ctx.clone(), url_host)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    url_obj
        .set("path", Function::new(ctx.clone(), url_path)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    url_obj
        .set("query", Function::new(ctx.clone(), url_query)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    url_obj
        .set("scheme", Function::new(ctx.clone(), url_scheme)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    url_obj
        .set("port", Function::new(ctx.clone(), url_port)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    url_obj
        .set("join", Function::new(ctx.clone(), url_join)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    url_obj
        .set("is_valid", Function::new(ctx.clone(), url_is_valid)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
...
fn url_join(base: String, path: String) -> Option<String> {
    Url::parse(&base)
        .ok()
        .and_then(|u| u.join(&path).ok().map(|u| u.to_string()))
}
