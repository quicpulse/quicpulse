...
pub fn register(ctx: &Ctx<'_>) -> Result<(), QuicpulseError> {
    ...
    assert_obj
        .set("eq", Function::new(ctx.clone(), assert_eq_fn)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    assert_obj
        .set("ne", Function::new(ctx.clone(), assert_ne_fn)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    assert_obj
        .set("is_true", Function::new(ctx.clone(), assert_true)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    assert_obj
        .set("is_false", Function::new(ctx.clone(), assert_false)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    assert_obj
        .set("gt", Function::new(ctx.clone(), assert_gt)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    assert_obj
        .set("gte", Function::new(ctx.clone(), assert_gte)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    assert_obj
        .set("lt", Function::new(ctx.clone(), assert_lt)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    assert_obj
        .set("lte", Function::new(ctx.clone(), assert_lte)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    assert_obj
        .set("ok", Function::new(ctx.clone(), assert_ok)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    assert_obj
        .set("fail", Function::new(ctx.clone(), assert_fail)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;

    // Status assertions
    assert_obj
        .set("status_ok", Function::new(ctx.clone(), assert_status_ok)?)
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    assert_obj
        .set(
            "status_success",
            Function::new(ctx.clone(), assert_status_success)?,
        )
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    assert_obj
        .set(
            "status_redirect",
            Function::new(ctx.clone(), assert_status_redirect)?,
        )
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    assert_obj
        .set(
            "status_client_error",
            Function::new(ctx.clone(), assert_status_client_error)?,
        )
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;
    assert_obj
        .set(
            "status_server_error",
            Function::new(ctx.clone(), assert_status_server_error)?,
        )
        .map_err(|e| QuicpulseError::Script(e.to_string()))?;

    globals
        .set("assert", assert_obj)
...
fn assert_status_client_error(status: i32) -> bool {
    if !(400..500).contains(&status) {
        eprintln!(
            "[ASSERT] Expected client error status (4xx), got {}",
            status
        );
        false
    } else {
...
fn assert_status_server_error(status: i32) -> bool {
    if !(500..600).contains(&status) {
        eprintln!(
            "[ASSERT] Expected server error status (5xx), got {}",
            status
        );
        false
    } else {
