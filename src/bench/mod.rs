...
pub fn format_results(result: &BenchmarkResult) -> String {
    ...
    output.push_str(&format!(
        "  {}              {}\n",
        terminal::label("URL:"),
        terminal::colorize(&result.url, colors::AQUA)
    ));
    output.push_str(&format!(
        "  {}           {}{}{}\n",
        terminal::label("Method:"),
    ...
    output.push_str(&format!(
        "  {}         {}\n",
        terminal::label("Requests:"),
        terminal::number(&result.total_requests.to_string())
    ));
    output.push_str(&format!(
        "  {}      {}\n",
        terminal::label("Concurrency:"),
        terminal::number(&result.concurrency.to_string())
    ));
    output.push_str(&format!(
        "  {}         {}\n",
    ...
    output.push_str(&format!(
        ...
        terminal::number(&format!("{:.2}", result.stats.requests_per_second))
    ));
    output.push_str(&format!(
        "  {}        {}\n",
        terminal::label("Bytes/sec:"),
        terminal::number(&crate::utils::format_bytes(
            result.stats.bytes_per_second as u64,
            2
        ))
    ));
    output.push_str(&format!(
    ...
    output.push_str(&format!(
        ...
        terminal::label("Min:"),
        terminal::colorize(
            &format!("{:.2}ms", result.stats.latency.min_ms),
            colors::GREEN
        )
    ));
    output.push_str(&format!(
        "  {}             {}\n",
        terminal::label("Mean:"),
        terminal::number(&format!("{:.2}ms", result.stats.latency.mean_ms))
    ));
    output.push_str(&format!(
        "  {}              {}\n",
        terminal::label("Max:"),
        terminal::colorize(
            &format!("{:.2}ms", result.stats.latency.max_ms),
            colors::ORANGE
        )
    ));
    output.push_str(&format!(
    ...
    output.push_str(&format!(
        "    {}            {}\n",
        terminal::muted("p50:"),
        terminal::number(&format!("{:.2}ms", result.stats.latency.p50_ms))
    ));
    output.push_str(&format!(
        "    {}            {}\n",
        terminal::muted("p75:"),
        terminal::number(&format!("{:.2}ms", result.stats.latency.p75_ms))
    ));
    output.push_str(&format!(
        "    {}            {}\n",
        terminal::muted("p90:"),
        terminal::number(&format!("{:.2}ms", result.stats.latency.p90_ms))
    ));
    output.push_str(&format!(
        "    {}            {}\n",
        terminal::muted("p95:"),
        terminal::number(&format!("{:.2}ms", result.stats.latency.p95_ms))
    ));
    output.push_str(&format!(
        "    {}            {}\n",
        terminal::muted("p99:"),
    ...
    output.push_str(&format!(
        ...
        terminal::colorize(
            &format!("{:.1}%", result.stats.success_rate * 100.0),
            success_color
        )
    ));
    let fail_color = if result.stats.failed_requests == 0 {
        colors::GREEN
    } else {
        colors::RED
    };
    output.push_str(&format!(
        ...
        terminal::colorize(&result.stats.failed_requests.to_string(), fail_color)
    ));
    ...
    if !result.stats.errors.is_empty() {
        ...
        for (error, count) in &result.stats.errors {
            output.push_str(&format!(
                "    {}: {}\n",
                terminal::colorize(error, colors::RED),
                terminal::number(&count.to_string())
            ));
        }
    }
                ...
                if let Ok(content) = std::fs::read_to_string(path) {
                    if let Ok(header_name) = reqwest::header::HeaderName::try_from(name.as_str()) {
                        if let Ok(header_value) =
                            reqwest::header::HeaderValue::from_str(content.trim())
                        {
                            headers.insert(header_name, header_value);
                        }
                    }
